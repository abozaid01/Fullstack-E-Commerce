<script lang="ts">
	...
	export let editCategoryModal = false;
	...
	const handleSubmit = async function () {
		...
		if (!name) errorsObj.name = `category's name is required!`;
		if (name.length < 3)
			errorsObj.name = `category's name must be at least 3 chracters`;
...
>
	<form on:submit|preventDefault={handleSubmit}>
		<div class="grid gap-4 sm:grid-cols-2 sm:gap-6">
			...
			<Button type="submit" class="w-32">Edit Category</Button>
		</div>
	</form>
</Modal>
