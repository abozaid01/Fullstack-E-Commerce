<script lang="ts">
	...
	import SubcategoryListItem from './SubcategoryListItem.svelte';
	import SubcategoryGridItem from './SubcategoryGridItem.svelte';
	...
	let paginationData = data.subcategories;

	let addSubategoryModal: boolean;
	let view = 'list';
	let searchTerm = '';
	...
	let totalItems = data.totalSubategories;
...
<AddSubcategoryModal bind:addSubategoryModal categories={data.categories} />
...
<div class="container flex flex-col gap-4">
	...
	>
		<Breadcrumb aria-label="Default breadcrumb example" navClass="mb-4">
			...
			<BreadcrumbItem>Subcategories</BreadcrumbItem>
		</Breadcrumb>
		...
		>
			...
			>
				...
				<Button on:click={() => (addSubategoryModal = true)}>
					<PlusOutline class="mr-2 h-3.5 w-3.5" />Add New Subategory
				</Button>
				...
				<TableBody tableBodyClass="divide-y">
					{#if searchTerm !== ''}
						...
						{#each filteredItems as item (item._id)}
							<SubcategoryListItem {item} categories={data.categories} />
						{/each}
					{:else}
			...
			{:else}
				...
				<div class="mb-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
					{#if searchTerm !== ''}
						<!-- SEARCHED GRID ITEMS -->
						{#each filteredItems as item (item._id)}
							<SubcategoryGridItem {item} categories={data.categories} />
						{/each}
					{:else}
						<!-- FULL GRID ITEMS -->
						{#each currentPageItems as item (item._id)}
