<script lang="ts">
	...
	const divClass = 'bg-white dark:bg-gray-800 relative shadow-md sm:rounded-lg overflow-x-scroll scrollbar-none';
	const innerDivClass =
	...
	// 				review: 'I love your products. It is very easy and fun to use this panel. I would recommend it to everyone.',
	// 				rate: 5.0
	// 			},
	// 			{
	...
	let paginationData = data.products;
	...
	$: filteredItems = paginationData.filter((item) => item.title.toLowerCase().indexOf(searchTerm.toLowerCase()) !== -1);
...
<AddProductModal bind:addProductModal brands={data.brands} subcategories={data.subcategories} />
				...
				<TableBody tableBodyClass="divide-y">
					{#if searchTerm !== ''}
						...
						{#each filteredItems as item (item._id)}
							<ProductListItem {item} brands={data.brands} subcategories={data.subcategories} />
						{/each}
					{:else}
			...
			{:else}
				...
				<div class="mb-8 grid grid-cols-2 gap-4 md:grid-cols-3 lg:grid-cols-4">
					{#if searchTerm !== ''}
						<!-- SEARCHED GRID ITEMS -->
						{#each filteredItems as item (item._id)}
							<ProductGridItem {item} brands={data.brands} subcategories={data.subcategories} />
						{/each}
					{:else}
						<!-- FULL GRID ITEMS -->
						{#each currentPageItems as item (item._id)}
			...
			>
				...
				<ButtonGroup divClass="flex gap-1">
					<Button
						...
						class="border-none bg-white p-2 dark:bg-gray-800"><ChevronLeftOutline size="md" /></Button
					>
					...
					<Button
						...
						class="border-none bg-white p-2 dark:bg-gray-800"><ChevronRightOutline size="md" /></Button
					>
				</ButtonGroup>
			</div>
		</TableSearch>
